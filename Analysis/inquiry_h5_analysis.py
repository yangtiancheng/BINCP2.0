...
def _get_operation(html):
    ...
    try:
        ...
        # 投标文件接收截止时间
        receiving_time_rule = re.compile(r'(递交时间：|截止时间：|截止日期：|日 期：|递交时间：|递交截止时间及询价时间：|时 间：)(.*)')
        ...
        if receiving_times:
            for i in range(len(receiving_times)):
                if len(receiving_times[i][1]) > 1:
                    ...
                    receiving_time_rule = re.compile(r'(递交时间：|截止时间：|投标截止日期：|日\s*期：|递交时间：|递交截止时间及询价时间：|时\s*间：)\n(.*)')
                    receiving_times = receiving_time_rule.findall(html)
                    receiving_time = receiving_times[i][1]
                    break
        else:
...
def _get_inquiry_info(max_number):
    """获取招标网站的信息"""
    info = []
    string = ''
    url_prefix = 'http://'
    ...
    if result:
        for each_lst in result:
            ...
            try:
                ...
                # # 询价时间
                # print "询价时间 " + result_info["p_begin_time"]
                # # 询价地点
                # print "询价地点 " + result_info["p_begin_addr"]
                #
                # # url
                # print "url " + url
