...
def _get_operation(html):
    ...
    try:
        ...
        if pro_purchase:
            ...
            for i in range(len(pro_purchase)):
                if len(pro_purchase[i][1]) > 1:
                    ...
                    variable_string.append(str(pro_purchase[i][1]))
                else:
                    pro_purchase_rule = re.compile(r'(采购项目联系人：|联系人：|联系人及电话：|联 系 人：|联  系  人：|电话/传真：)\n(.*)')
                    pro_purchase = pro_purchase_rule.findall(html)
                    variable_string.append(str(pro_purchase[i][1]))
        ...
        # 联系方式(最后一个一定是采购人联系方式)
        contact_info_rule = re.compile(r'(联系方式：|联系电话：|联系方式（电话/传真）：|联系方式（传真）：|联系方式（电话）：|电话/传真：)(.*)')
        ...
        if contact_infos:
            ...
            for i in range(len(contact_infos)):
                if len(contact_infos[i][1]) > 1:
                    # print "采购项目联系人:" + pro_purchase[i][1]
                    variable_string.append(str(contact_infos[i][1]))
                else:
                    contact_info_rule = re.compile(r'(联系方式：|联系电话：|联系方式（电话/传真）：|联系方式（传真）：|联系方式（电话）：|电话/传真：)\n(.*)')
                    contact_infos = contact_info_rule.findall(html)
                    variable_string.append(str(contact_infos[i][1]))
        ...
        # 投标文件接收截止时间
        receiving_time_rule = re.compile(r'(截止时间：|投标截止日期：|日 期：|递交时间：|日期：|递交截止时间及谈判时间：)(.*)')
        receiving_time = receiving_time_rule.findall(html)
        if receiving_time:
            for i in range(len(receiving_time)):
                if len(receiving_time[i][1]) > 1:
                    # print "文件接收截止时间:" + receiving_time[i][1]
                    receiving_time = receiving_time[i][1]
                    break
                else:
                    receiving_time_rule = re.compile(r'(截止时间：|投标截止日期：|日 期：|日期：|递交时间：|递交截止时间及谈判时间：)\n(.*)')
                    receiving_time = receiving_time_rule.findall(html)
                    receiving_time = receiving_time[i][1]
                    break
        else:
        ...
        # 谈判时间
        begin_time_rule = re.compile(r'(谈判时间：|时间：)(.*)')
        ...
        # 谈判地点
        begin_addr_rule = re.compile(r'(谈判地点：|地址：|竞谈地点：|召开地点：|大会地点：)(.*)')
...
def _get_talks_info(max_number):
    """获取招标网站的信息"""
    info = []
    string = ''
    url_prefix = 'http://'
    ...
    if result:
        for each_lst in result:
            ...
            try:
                ...
                # # 谈判时间
                # print "谈判时间 " + result_info["p_begin_time"]
                # # 谈判地点
                # print "谈判地点 " + result_info["p_begin_addr"]
                #
                # # url
                # print "url " + url
                #
