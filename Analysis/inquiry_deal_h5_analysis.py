...
def _get_operation(html):
    ...
    try:
        ...
        hitted_orgaization_name_rule = re.compile(
            r'(服务商：|招标结果：|中标社会资本方：|成交企业：|单位排序如下:|供\*s应\*s商\d：|成交单位：|成交单位\d：|成交服务商：|供应商名称：|供\*s应\*s商：|成交人：|报价人：|候选人：|供应商候选人排序：|成交供应商：|单位如下：|预审单位名单如下：|中标单位：|中标供应商：|中标供应商名称：|中标人：|供应商：|第一名：|中标服务商：|成交单位：|入围单位：|中\s*标\s*人：)(.*)')
            ...
            for i in range(len(hitted_orgaization_names)):
                if len(hitted_orgaization_names[i][1]) > 1:
                    ...
                    hitted_orgaization_name_rule2 = re.compile(
                        r'(服务商：|招标结果：|：|成交企业：|单位排序如下:|成交供应商：|供应商\d：|成交单位：|成交单位\d：|成交服务商：|供应商名称：|供应商：|成交人：|报价人：|候选人：|供应商候选人排序：|成交供应商：|单位如下：|预审单位名单如下：|中标单位：|中标供应商：|中标供应商名称：|中标人：|供应商：|第一名：|中标服务商：|成交单位：|入围单位：|中 标 人：|中标人：)\n(.*)')
        ...
        judgor_name_rule = re.compile(
            r'(评标委员会成员名单：|评标委员会成员名单：|成员名单：|评标委员会成员：|评标小组成员：|成员：|小组人员：|人员：|评标委员会人员：|评标委员.*）：|评标委员会名单：|评审专家|专家|评标委员会：|委员会：|委员： |人员名单：|磋商小组：|小组名单：|名单：|小组成员：)(.*)')
        judgor_names = judgor_name_rule.findall(html)
        if judgor_names:
            for i in range(len(judgor_names)):
                if len(judgor_names[i][1]) > 1 and judgor_names[i][1] != '\r':
                    ...
                    judgor_name_rule2 = re.compile(
                        r'(评标委员会成员名单：|评标委员会成员名单：|成员名单：|评标委员会成员：|评标小组成员：|成员：|小组人员：|人员：|评标委员会人员：|评标委员.*）：|评标委员会名单：|评审专家|专家|评标委员会：|委员会：|委员：|人员名单：|磋商小组：|小组名单：|名单：|小组成员：)\n(.*)')
                    judgor_names = judgor_name_rule2.findall(html)
                    if judgor_names:
        ...
        pro_purchase1 = pro_purchase_rule1.findall(html)
        pro_purchase2 = pro_purchase_rule2.findall(html)
...
def _get_inquiry_deal_info(max_number):
    """获取招标网站的信息"""
    info = []
    string = ''
    url_prefix = 'http://'
